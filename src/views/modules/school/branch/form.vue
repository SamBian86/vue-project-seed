      ...
      >
        <template slot="footer">
          <div class="form-submit-container">
            ...
            >{{ $t('back') }}</el-button>
            <el-button
              v-if="containsPageType(['create']) && filterPermission('school:branch:save')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('add') }}</el-button>
            <el-button
              v-if="containsPageType(['edit']) && filterPermission('school:branch:update')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('update') }}</el-button>
...
import { createSchoolBranch, editSchoolBranch, getSchoolBranchById } from '@/api/school/branch'
  ...
  data() {
    return {
      ...
      formHandle: {
        // 创建抽象方法，用创建接口方法覆盖
        create: {
          api: createSchoolBranch
        },
        // 修改抽象方法，用修改接口方法覆盖
        edit: {
          api: editSchoolBranch
        },
        // 详情抽象方法，用详情接口方法覆盖
        detail: {
          api: getSchoolBranchById
        }
      },
      // 初始化数据定义
      formDefaultData: {},
