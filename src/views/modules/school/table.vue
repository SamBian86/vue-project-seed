    ...
    >
      ...
      <template slot="search">
        ...
        >
          <el-form-item>
            <el-input
              v-model="tableSearchParams.schoolName"
              :placeholder="$t('school.schoolName')"
          ...
          <el-form-item>
            <el-select
              v-model="tableSearchParams.schoolStatus"
              :placeholder="$t('school.schoolStatus')"
              :size="tableConfig.tableSearchSize"
              clearable
              @clear="clearHandle"
            >
              <el-option
                v-for="(item, index) in getDictByType('schoolStatus')"
                :key="index"
          ...
          <el-form-item>
            <el-button
              v-if="filterPermission('school:save')"
              type="primary"
              :size="tableConfig.tableSearchSize"
              @click="createHandle()"
            >{{ $t('add') }}</el-button>
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('school:export')"
              type="primary"
              :size="tableConfig.tableSearchSize"
              @click="exportHandle()"
            >{{ $t('export') }}</el-button>
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('school:delete')"
              type="danger"
              :size="tableConfig.tableSearchSize"
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('school:xxx')"
              type="danger"
              :size="tableConfig.tableSearchSize"
      ...
      <template slot="operate">
        ...
        >
          <template slot-scope="scope">
            <!-- 上下架 -->
            <!--<el-button
              v-if="filterPermission('school:display')"
              type="text"
              :size="tableConfig.tableSearchSize"
              @click="customHandle({
                data: {
            ...
            <!-- 修改 -->
            <el-button
              v-if="filterPermission('school:update')"
              type="text"
              :size="tableConfig.tableSearchSize"
            ...
            <!-- 上移 -->
            <!--<el-button
              v-if="filterPermission('school:exchange') && scope.$index !== 0"
              type="text"
              :size="tableConfig.tableSearchSize"
            ...
            <!-- 下移 -->
            <!--<el-button
              v-if="filterPermission('school:exchange') && scope.$index !== limit - 1"
              type="text"
              :size="tableConfig.tableSearchSize"
            ...
            <!-- 停用、启用 -->
            <el-button
              v-if="filterPermission('school:status')"
              type="text"
              :size="tableConfig.tableSearchSize"
              @click="customHandle({
                data: {
                  id: scope.row.id,
                  display: scope.row.schoolStatus === 0 ? 1 : 0
                },
                i18nRequestCustomMessage: scope.row.schoolStatus === 0 ? 'school.schoolStatus1Message' : 'school.schoolStatus0Message',
                request: setSchoolDisplay
              })"
            >{{ scope.row.schoolStatus === 0 ? $t('school.schoolStatus1') : $t('school.schoolStatus0') }}</el-button>
            <!-- 学院设置 -->
            <el-button
              v-if="filterPermission('school:set')"
              type="text"
              :size="tableConfig.tableSearchSize"
              @click="setSchoolBranchHandle(scope.row)"
            >{{ $t('school.set') }}</el-button>
          </template>
        </el-table-column>
      </template>
    </yunlin-table>
...
import { getSchoolPageList, setSchoolDisplay } from '@/api/school'
  ...
  methods: {
    ...
    init() {
      ...
      this.tableConfig.tableHeadReadOnly = [
        // 学校名称
        { prop: 'schoolName', label: 'school.schoolName' },
        // 联系地址
        { prop: 'schoolAddress', label: 'school.schoolAddress' },
        // 学校状态
        {
          prop: 'schoolStatus',
          label: 'school.schoolStatus',
          width: '160',
          align: 'center',
          component: 'toolTag',
          componentConfig: {
            prop: 'schoolStatus',
            type: 'text',
            tagConfig: this.dictDataForComponent('schoolStatus', { dictLabel: 'name', dictValue: 'value' })
          }
        }
      ]
      // 是否填充查询条件为空
      ...
      // 配置列表请求
      this.tableHandle.list.api = getSchoolPageList
    ...
    // return XXX
    // }
    setSchoolBranchHandle(row) {
      this.setDrawerComponent('branch')
      this.setDrawerData(row)
      this.setDrawerTitle(this.$t('school.set'))
