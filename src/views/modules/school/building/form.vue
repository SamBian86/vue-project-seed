      ...
      >
        <template slot="footer">
          <div class="form-submit-container">
            ...
            >{{ $t('back') }}</el-button>
            <el-button
              v-if="containsPageType(['create']) && filterPermission('school:building:save')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('add') }}</el-button>
            <el-button
              v-if="containsPageType(['edit']) && filterPermission('school:building:update')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('update') }}</el-button>
...
import { createSchoolBuilding, editSchoolBuilding, getSchoolBuildingById } from '@/api/school/building'
  ...
  data() {
    return {
      ...
      formHandle: {
        // 创建抽象方法，用创建接口方法覆盖
        create: {
          api: createSchoolBuilding
        },
        // 修改抽象方法，用修改接口方法覆盖
        edit: {
          api: editSchoolBuilding
        },
        // 详情抽象方法，用详情接口方法覆盖
        detail: {
          api: getSchoolBuildingById
        }
      },
      // 初始化数据定义
      formDefaultData: {},
      ...
      this.formConfig.formItemsReadOnly = [
        {
          ...
          span: 24,
          prop: 'buildingName',
          name: 'schoolBuilding.buildingName',
          type: 'text',
          rules: [{ required: true }],
          attrs: { maxlength: 20 }
        },
        {
          ...
          span: 24,
          prop: 'buildingType',
          name: 'schoolBuilding.buildingType',
          type: 'select',
          className: 'select-block',
          placeholder: 'schoolBuilding.buildingType',
          rules: [{ required: true }],
          items: this.getDictByType('buildingType'),
          itemType: 'dict'
        }
      ]
