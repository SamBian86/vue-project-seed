      ...
      >
        <template slot="footer">
          <div class="form-submit-container">
            ...
            >{{ $t('back') }}</el-button>
            <el-button
              v-if="containsPageType(['create']) && filterPermission('sys:platform-user:save')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('add') }}</el-button>
            <el-button
              v-if="containsPageType(['edit']) && filterPermission('sys:platform-user:update')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('update') }}</el-button>
...
import { createPlatformUser, editPlatformUser, getPlatformUserById } from '@/api/sys/platformUser'
  ...
  data() {
    return {
      ...
      formHandle: {
        ...
        // 修改抽象方法，用修改接口方法覆盖
        edit: {
          api: editPlatformUser
        },
        // 详情抽象方法，用详情接口方法覆盖
        detail: {
          api: getPlatformUserById
        }
      },
      // 初始化数据定义
      formDefaultData: {},
    ...
    init() {
      ...
      this.formConfig.formItemsReadOnly = [
        ...
        },
        {
          ...
          span: 24,
          prop: 'roleId',
          name: 'sysPlatformUser.roleId',
          type: 'select-dynamic',
          rules: [{ required: true }],
          component: 'toolSelectDynamic',
          componentConfig: {
            request: getTenantRoleList,
            requestParams: {},
            itemProps: {
              label: 'name',
              value: 'id'
            ...
            propName: 'roleId',
            placeholder: 'sysPlatformUser.roleId',
