        ...
        >
          <el-form-item>
            <el-input
              ...
              :placeholder="$t('sysTenantRole.name')"
          ...
          <el-form-item>
            <el-button
              v-if="filterPermission('sys:tenant-role:view')"
              :size="tableConfig.tableSearchSize"
              @click="clearHandle"
            >{{ $t('query') }}</el-button>
          </el-form-item>
          ...
          <el-form-item>
            <el-button
              v-if="filterPermission('sys:tenant-role:save')"
              type="primary"
              :size="tableConfig.tableSearchSize"
              @click="createHandle()"
            >{{ $t('add') }}</el-button>
          ...
          <!--<el-form-item>
            <el-button
              v-if="filterPermission('sys:tenant-role:export')"
              type="primary"
              :size="tableConfig.tableSearchSize"
              @click="exportHandle()"
            >{{ $t('export') }}</el-button>
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('sys:tenant-role:delete')"
              type="danger"
              :size="tableConfig.tableSearchSize"
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('sys:tenant-role:xxx')"
              type="danger"
              :size="tableConfig.tableSearchSize"
      ...
      <template slot="operate">
        ...
        >
          <template slot-scope="scope">
            <!-- 上下架 -->
            <!--<el-button
              v-if="filterPermission('sys:tenant-role:display')"
              type="text"
              :size="tableConfig.tableSearchSize"
              @click="customHandle({
                data: {
            ...
            <!-- 修改 -->
            <el-button
              v-if="filterPermission('sys:tenant-role:update')"
              type="text"
              :size="tableConfig.tableSearchSize"
            ...
            <el-button
              ...
              type="text"
              :size="tableConfig.tableSearchSize"
              @click="deleteHandle({id: scope.row.id, i18nMessage: 'sysTenantRole.deleteMessage'})"
            >{{ $t('delete') }}</el-button>
            <!-- 上移 -->
            <!--<el-button
              v-if="filterPermission('sys:tenant-role:exchange') && scope.$index !== 0"
              type="text"
              :size="tableConfig.tableSearchSize"
            ...
            <!-- 下移 -->
            <!--<el-button
              v-if="filterPermission('sys:tenant-role:exchange') && scope.$index !== limit - 1"
              type="text"
              :size="tableConfig.tableSearchSize"
...
import { getTenantRolePageList, deleteTenantRole } from '@/api/sys/tenantRole'
  ...
  methods: {
    ...
    init() {
      ...
      this.tableConfig.tableHeadReadOnly = [
        // 角色名称
        { prop: 'name', label: 'sysTenantRole.name' }
      ]
      // 是否填充查询条件为空
      // this.tableConfig.searchFillEmpty = true
      // this.tableSearchParams = {}
      // 配置列表请求
      this.tableHandle.list.api = getTenantRolePageList
      // this.tableHandle.list.callback = this.listCallback
      // 配置导出功能
      // this.tableHandle.export.api = exportXXX
      // 配置删除功能
      this.tableHandle.delete.api = deleteTenantRole
      // this.tableHandle.delete.callback = this.deleteCallback
      // 配置节点懒加载功能
      // this.tableHandle.lazy.api = lazyXXX
      // 配置section删除功能
      // this.tableHandle.deleteSection.api = deleteTenantRole
