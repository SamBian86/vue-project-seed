        ...
        <el-form class="table-search-form" :inline="true" :model="tableSearchParams" @keyup.enter.native="searchHandle">
          <el-form-item>
            <el-input
              ...
              :placeholder="$t('regulationsCheck.userName')"
          ...
          <el-form-item>
            <el-input
              ...
              :placeholder="$t('regulationsCheck.roomName')"
              :size="tableConfig.tableSearchSize"
              clearable
              @clear="clearHandle"
            ></el-input>
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('regulations:check:save')"
              type="primary"
              :size="tableConfig.tableSearchSize"
              @click="createHandle()"
            >
          ...
          <!--<el-form-item>
            <el-button
              v-if="filterPermission('regulations:check:downloadExcel')"
              type="success"
              :size="tableConfig.tableSearchSize"
          ...
          <!-- 导入 -->
          <!--<el-form-item>
            <button-import v-if="filterPermission('regulations:check:importExcel')" :config="importConfig"></button-import>
          </el-form-item>-->
          <!-- 清空选中 -->
          <!--<el-form-item>
            <el-button
          ...
          <el-form-item>
            <el-button
              v-if="filterPermission('regulations:check:export')"
              type="primary"
              :size="tableConfig.tableSearchSize"
              @click="exportHandle()"
            >
          ...
          <!-- <el-form-item>
            <el-button
              v-if="filterPermission('regulations:check:xxx')"
              type="danger"
              :size="tableConfig.tableSearchSize"
      ...
      <template slot="operate">
        <el-table-column :label="$t('handle')" align="center" header-align="center" fixed="right" width="100">
          <template slot-scope="scope">
            <!-- 上下架 -->
            <!--<el-button
              v-if="filterPermission('regulations:check:display')"
              type="text"
              :size="tableConfig.tableSearchSize"
              @click="customHandle({
                data: {
            ...
            <!-- 下移 -->
            <!--<el-button
              v-if="filterPermission('regulations:check:exchange') && scope.$index !== limit - 1"
              type="text"
              :size="tableConfig.tableSearchSize"
...
import { getRegulationsCheckPageList, exportRegulationsCheck } from '@/api/regulations/check'
  ...
  methods: {
    ...
    init() {
      ...
      this.tableConfig.tableHeadReadOnly = [
        // 检查时间
        { prop: 'createDate', label: 'regulationsCheck.createDate', width: '160' },
        // 检查人
        { prop: 'userName', label: 'regulationsCheck.userName', width: '100' },
        // 宿舍楼栋
        { prop: 'buildingName', label: 'regulationsCheck.buildingName', width: '120' },
        // 楼层
        { prop: 'floorName', label: 'regulationsCheck.floorName', width: '120' },
        // 宿舍号
        { prop: 'roomName', label: 'regulationsCheck.roomName', width: '120' },
        // 违章人
        { prop: 'studentName', label: 'regulationsCheck.studentName' }
      ]
      // 是否填充查询条件为空
      // this.tableConfig.searchFillEmpty = true
      // this.tableSearchParams = {}
      // 配置列表请求
      this.tableHandle.list.api = getRegulationsCheckPageList
      // this.tableHandle.list.callback = this.listCallback
      // 配置导出功能
      this.tableHandle.export.api = exportRegulationsCheck
      // 配置删除功能
