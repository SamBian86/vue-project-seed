    ...
    >
      ...
      <template slot="search">
        ...
        >
          ...
          <el-form-item>
            <el-select
              v-model="tableSearchParams.position"
              :placeholder="$t('maintenanceWorker.position')"
              :size="tableConfig.tableSearchSize"
              clearable
              @clear="clearHandle"
            >
              <el-option
                v-for="(item, index) in getDictByType('maintenancePosition')"
                :key="index"
          ...
          <el-form-item>
            <el-input
              v-model="tableSearchParams.deptName"
              :placeholder="$t('maintenanceWorker.deptName')"
              :size="tableConfig.tableSearchSize"
              clearable
              @clear="clearHandle"
            ></el-input>
          ...
          <el-form-item>
            <el-input
              ...
              :placeholder="$t('maintenanceWorker.buildingName')"
              :size="tableConfig.tableSearchSize"
              clearable
              @clear="clearHandle"
            ></el-input>
      ...
      <template slot="operate">
        ...
        >
          <template slot-scope="scope">
            ...
            <!-- 修改 -->
            <el-button
              v-if="filterPermission('maintenance:worker:update')"
              type="text"
              :size="tableConfig.tableSearchSize"
...
import { getMaintenanceWorkerPageList, deleteMaintenanceWorker, getMaintenanceWorkerQRcode } from '@/api/maintenance/worker'
  ...
  methods: {
    ...
    init() {
      ...
      this.tableConfig.tableHeadReadOnly = [
        // 维修人员
        { prop: 'employeeName', label: 'maintenanceWorker.employeeName', width: '160' },
        // 手机号
        { prop: 'phone', label: 'maintenanceWorker.phone', width: '160' },
        // 人员类型
        {
          prop: 'position',
          label: 'maintenanceWorker.position',
          width: '120',
          align: 'center',
          component: 'toolTag',
          componentConfig: {
            prop: 'position',
            type: 'text',
            tagConfig: this.dictDataForComponent('maintenancePosition', { dictLabel: 'name', dictValue: 'value' })
          }
        },
        // 所属组织
        { prop: 'deptName', label: 'maintenanceWorker.deptName', width: '120' },
        // 服务楼栋
        { prop: 'buildingNameStr', label: 'maintenanceWorker.buildingNameStr' }
      ]
      // 是否填充查询条件为空
      // this.tableConfig.searchFillEmpty = true
      // this.tableSearchParams = {}
      // 配置列表请求
      this.tableHandle.list.api = getMaintenanceWorkerPageList
      // this.tableHandle.list.callback = this.listCallback
      // 配置导出功能
      // this.tableHandle.export.api = exportXXX
      // 配置删除功能
      this.tableHandle.delete.api = deleteMaintenanceWorker
      // this.tableHandle.delete.callback = this.deleteCallback
      // 配置节点懒加载功能
      // this.tableHandle.lazy.api = lazyXXX
      // 配置section删除功能
      // this.tableHandle.deleteSection.api = deleteMaintenanceWorker
