      ...
      >
        <template slot="footer">
          <div class="form-submit-container">
            ...
            >{{ $t('back') }}</el-button>
            <el-button
              v-if="containsPageType(['create']) && filterPermission('engineering:change:type:save')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('add') }}</el-button>
            <el-button
              v-if="containsPageType(['edit']) && filterPermission('engineering:change:type:update')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('update') }}</el-button>
...
import {
  createEngineeringContractChangeType,
  editEngineeringContractChangeType,
  getEngineeringContractChangeTypeById
  ...
  data() {
    return {
      ...
      formHandle: {
        // 创建抽象方法，用创建接口方法覆盖
        create: {
          api: createEngineeringContractChangeType
        },
        // 修改抽象方法，用修改接口方法覆盖
        edit: {
          api: editEngineeringContractChangeType
        },
        // 详情抽象方法，用详情接口方法覆盖
        detail: {
          api: getEngineeringContractChangeTypeById
        }
      },
      // 初始化数据定义
      formDefaultData: {},
      ...
      this.formConfig.formItemsReadOnly = [
        {
          ...
          span: 24,
          prop: 'changeTypeName',
          name: 'contractChangeType.changeTypeName',
          type: 'text',
          rules: [{ required: true }]
        },
        {
          ...
          span: 24,
          prop: 'remark',
          name: 'contractChangeType.remark',
          type: 'textarea',
