      ...
      >
        <template slot="footer">
          <div class="form-submit-container">
            ...
            >{{ $t('back') }}</el-button>
            <el-button
              v-if="containsPageType(['create']) && filterPermission('engineering:projectarea:save')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('add') }}</el-button>
            <el-button
              v-if="containsPageType(['edit']) && filterPermission('engineering:projectarea:update')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('update') }}</el-button>
...
import {
  createEngineeringProjectarea,
  editEngineeringProjectarea,
  getEngineeringProjectareaById
} from '@/api/engineering/projectarea'
  ...
  data() {
    return {
      ...
      formHandle: {
        // 创建抽象方法，用创建接口方法覆盖
        create: {
          api: createEngineeringProjectarea
        },
        // 修改抽象方法，用修改接口方法覆盖
        edit: {
          api: editEngineeringProjectarea
        },
        // 详情抽象方法，用详情接口方法覆盖
        detail: {
          api: getEngineeringProjectareaById
        }
      },
      // 初始化数据定义
      formDefaultData: {},
      ...
      this.formConfig.formItemsReadOnly = [
        ...
        },
        {
          ...
          componentConfig: {
            ...
            treeDataFilter: true,
            treeDataFilterKey: '',
            i18nDefault: 'projectarea.deptName',
            propName: 'deptName',
            sourceName: 'name',
