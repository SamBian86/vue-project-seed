<template>
  <el-row :gutter="10">
    <el-col
      :span="formConfig.formSpan"
      :lg="formConfig.formSpan"
      :md="formConfig.formSpan"
      :sm="24"
      :xs="24"
    >
      <div
        v-if="formGenerateTitle[$attrs.page_info.data.pageType] !== ''"
        class="form-title"
      >{{ formGenerateTitle[$attrs.page_info.data.pageType] }}</div>
      <yunlin-form
        ref="yunlinForm"
        :config="formConfig"
        :handle="formHandle"
        :data="formData"
        :rules="formRules"
        v-bind="$attrs"
        @form-data-merge="formDataMerge"
        @form-generate-rule-by-props="formGenerateRuleByProps"
        @form-value-listener="formValueListener"
        @form-data-update="formDataUpdate"
        @form-reset-config-item="formResetConfigItem"
        v-on="$listeners"
      >
        <template slot="footer">
          <div class="form-submit-container">
            <el-button
              v-if="containsPageType(['create', 'edit', 'detail'])"
              :size="formConfig.formSize"
              @click.stop="cancleHandle"
            >{{ $t('back') }}</el-button>
            <el-button
              v-if="containsPageType(['create']) && filterPermission('property:employee:save')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('add') }}</el-button>
            <el-button
              v-if="containsPageType(['edit']) && filterPermission('property:employee:update')"
              type="primary"
              :size="formConfig.formSize"
              @click.stop="submitHandle"
            >{{ $t('update') }}</el-button>
          </div>
        </template>
      </yunlin-form>
    </el-col>
  </el-row>
</template>

<script>
import { mapGetters } from 'vuex'
import commonMixin from '@/mixins/common-mixin'
import pageMixin from '@/mixins/page-mixin'
import formDefaultMixin from '@/mixins/form-default-mixin'
import { getSysRoleList } from '@/api/sys/role'
import { getSysDeptListAll } from '@/api/sys/dept'
import { createPropertyEmployee, editPropertyEmployee, getPropertyEmployeeById } from '@/api/property/employee'
import { getProjectList } from '@/api/project'
import { validateMobile } from '@/utils/validator'

export default {
  name: 'Form',
  components: {},
  mixins: [commonMixin, pageMixin, formDefaultMixin],
  data() {
    return {
      // 定义表单名称
      formTitle: {
        create: this.$t('add'),
        edit: this.$t('update'),
        detail: this.$t('detail')
      },
      formGenerateTitle: {},
      formHandle: {
        // 创建抽象方法，用创建接口方法覆盖
        create: {
          api: createPropertyEmployee
        },
        // 修改抽象方法，用修改接口方法覆盖
        edit: {
          api: editPropertyEmployee
        },
        // 详情抽象方法，用详情接口方法覆盖
        detail: {
          api: getPropertyEmployeeById
        }
      },
      // 初始化数据定义
      formDefaultData: {},
      // 用于处理表单的隐藏与显示禁用行为
      formAction: [
        { prop: 'roleId', exclude: [{ value: '2', props: ['roleId', 'frontProjectIdList', 'projectIdList', 'deptName'] }] }
      ]
    }
  },
  computed: {
    ...mapGetters('app', ['filterPermission', 'getDictByType', 'getDictNameByValue'])
  },
  activated() {
    // console.log('form activated')
    this.init()
  },
  created() {
    // console.log(this.$attrs.page_info)
    // console.log('form created')

    // 设置整体表单栅格列数
    this.formConfig.formSpan = 12
  },
  methods: {
    generateTitle() {
      const { formTitle } = this
      this.formGenerateTitle = formTitle
    },
    init() {
      // 设置表单内容
      this.formConfig.formItemsReadOnly = [
        {
          // 姓名 15
          span: 24,
          prop: 'name',
          name: 'propertyEmployee.name',
          type: 'text',
          rules: [{ required: true }],
          attrs: { maxlength: 15 }
        },
        {
          // 手机号 11
          span: 24,
          prop: 'phoneNumber',
          name: 'propertyEmployee.phoneNumber',
          type: 'text',
          rules: [{ required: true }, { validator: validateMobile, trigger: 'blur' }],
          attrs: { maxlength: 11 }
        },
        {
          // 所属组织
          span: 24,
          prop: 'deptName',
          name: 'propertyEmployee.deptName',
          type: 'popover-tree',
          rules: [{ required: true }],
          component: 'toolPopoverTree',
          componentConfig: {
            request: getSysDeptListAll,
            requestParams: {},
            // treeDataTranslate,
            // treeDataFilter: true,
            // treeDataFilterKey: '',
            i18nDefault: 'propertyEmployee.deptName',
            propName: 'deptName',
            sourceName: 'name',
            treeProps: { label: 'name', children: 'children' },
            treeNodeKey: 'id',
            mergeData: [
              { source: 'name', target: 'deptName', default: '' },
              { source: 'id', target: 'deptId', default: 0 }
            ],
            componentNames: ['popover-tree']
          }
        },
        {
          // 岗位
          span: 24,
          prop: 'roleId',
          name: 'propertyEmployee.roleId',
          type: 'select-dynamic',
          rules: [{ required: true }],
          component: 'toolSelectDynamic',
          componentConfig: {
            request: getSysRoleList,
            requestParams: {},
            itemProps: {
              label: 'name',
              value: 'id'
            },
            propName: 'roleId',
            placeholder: 'propertyEmployee.roleId',
            className: 'select-block',
            mergeData: { target: 'roleId' },
            componentNames: ['select-dynamic']
          }
        },
        {
          // 人员性质
          span: 24,
          prop: 'type',
          name: 'propertyEmployee.type',
          type: 'select',
          className: 'select-block',
          placeholder: 'propertyEmployee.type',
          items: this.getDictByType('employeeType'),
          itemType: 'dict'
        },
        {
          // 身份证号
          span: 24,
          prop: 'idCard',
          name: 'propertyEmployee.idCard',
          type: 'text'
        },
        {
          // 户籍地址
          span: 24,
          prop: 'censusRegister',
          name: 'propertyEmployee.censusRegister',
          type: 'text',
          attrs: { maxlength: 50 }
        },
        {
          // 当前住址
          span: 24,
          prop: 'address',
          name: 'propertyEmployee.address',
          type: 'text',
          attrs: { maxlength: 50 }
        },
        {
          // 所属项目
          span: 12,
          prop: 'frontProjectIdList',
          name: 'propertyEmployee.frontProjectIdList',
          type: 'tree-dynamic',
          component: 'toolTreeDynamic',
          componentConfig: {
            treeRequest: getProjectList,
            treeRequestParams: {},
            // treeResultRequest: null,
            // treeResultRequestParams: {},
            // treeResultRequestPropParams: ['id'],
            // treeResultKey: '',
            propName: 'frontProjectIdList',
            nodeKey: 'id',
            treeProps: { label: 'projectName', children: 'children' },
            mergeData: { target: 'frontProjectIdList' },
            accordion: true,
            showCheckbox: true,
            componentNames: ['tree-dynamic']
          }
        },
        {
          // 项目数据权限
          span: 12,
          prop: 'projectIdList',
          name: 'propertyEmployee.projectIdList',
          type: 'tree-dynamic',
          component: 'toolTreeDynamic',
          componentConfig: {
            treeRequest: getProjectList,
            treeRequestParams: {},
            // treeResultRequest: null,
            // treeResultRequestParams: {},
            // treeResultRequestPropParams: ['id'],
            // treeResultKey: '',
            propName: 'projectIdList',
            nodeKey: 'id',
            treeProps: { label: 'projectName', children: 'children' },
            mergeData: { target: 'projectIdList' },
            accordion: true,
            showCheckbox: true,
            componentNames: ['tree-dynamic']
          }
        }
      ]

      // 组装表单初始数据
      this.generateFormData()
      // 生成标题
      this.generateTitle()
      // 生成表单及验证规则
      this.generateForm()
    }
  }
}
</script>
